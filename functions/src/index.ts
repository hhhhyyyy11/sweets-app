...
async function handleConsumeCommand(userId: string, text: string): Promise<void> {
...
async function notifyLowStock(sweetId: string, sweetName: string, currentStock: number): Promise<void> {
  // 管理者に通知
...
app.get("/sweets", async (req, res) => {
  try {
    ...
    const sweets = sweetsSnapshot.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));
...
export const lineBotWebhook = onRequest(async (req, res) => {
  ...
  if (!channelSecret) {
    ...
    try {
      const isValid = validateSignature(bodyString, channelSecret, signature);
 ...
 * LINE Botイベントハンドラー
 */
async function handleBotEvent(event: WebhookEvent): Promise<void> {
  // テキストメッセージのみ処理
  if (event.type !== "message" || event.message.type !== "text") {
 ...
 * 管理者にリマインダー送信結果を通知
 */
async function notifyAdminsAboutReminder(
  successCount: number,
  failureCount: number,
  totalAmount: number
): Promise<void> {
